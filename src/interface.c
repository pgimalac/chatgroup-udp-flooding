...
const char *pseudos[28] = {"Raskolnikov",     "Mlle Swann",
                           "Joshep K.",       "Humbert Humbert",
                           "Jacopo Belbo",    "Méphistophélès",
                           "Hamlet",          "Alcèste",
                           "Cthulhu",         "Samsaget Gamgie",
                           "Thomas Anderson", "Walter White",
                           "Wednesday",       "Morty",
                           "Dexter",          "The eleventh Doctor",
                           "Elliot Alderson", "Doctor House",
                           "Ragnar Lodbrok",  "Hannibal",
                           "Sherlock",        "King Lear",
                           "Zarathustra",     "Deep Thought",
                           "Arthur Dent",     "Tyler Durden",
                           "John Wayne",      "HAL"};
...
static void nameRandom(const char *buffer, size_t len) { setRandomPseudo(); }
...
static void switchlog(const char *buffer, size_t len) {
    ...
    if (bufferbis == NULL) {
        ...
        buf[len] = '\0';
        logfd =
            open(buf, O_WRONLY | O_CREAT | O_EXCL, S_IRWXO | S_IRWXG | S_IRWXU);
        if (logfd == -1) {
            cperror("open");
            cprint(STDOUT_FILENO, "Log are now writen on STDERR.\n");
            logfd = STDERR_FILENO;
...
static const char *names[] = {
    "add",  "name",      "random",    "print", "juliusz", "neighbour", "clear",
    "chid", "transfert", "switchlog", "help",  "quit",    NULL};
...
static void (*interface[])(const char *, size_t) = {
    add,  name,      nameRandom, print, juliusz, neighbour, clear,
    chid, transfert, switchlog,  help,  quit,    NULL};
...
const char *getPseudo() { return pseudo; }
...
void print_message(const u_int8_t *buffer, int size) {
    ...
    if (is_utf8(buffer, size))
        cprint(STDOUT_FILENO, "%*d:%*d:%*d > %*s\n", 2, t->tm_hour, 2,
               t->tm_min, 2, t->tm_sec, size, buffer);
    else
        cprint(STDOUT_FILENO, "%*d:%*d:%*d > (MALFORMED UTF8)\n", 2, t->tm_hour,
               2, t->tm_min, 2, t->tm_sec);
}
