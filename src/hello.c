    ...
    for (size_t i = 0; i < potential_neighbours->capacity; i++) {
        for (list_t *l = potential_neighbours->tab[i]; l != NULL; l = l->next) {
            ...
            if (p->short_hello_count >= NBSH) {
                inet_ntop(AF_INET6, &p->addr->sin6_addr, ipstr,
                          INET6_ADDRSTRLEN);
            ...
            if (delta >= max) {
                ...
                if (rc < 0) {
                    cprint(STDERR_FILENO,
                           "Could not insert short hello into message queue\n");
                    free(hello->content);
                    free(hello);
                }
    ...
    while (to_delete != NULL) {
        ...
        if (n->tutor_id) {
            ...
            if (m) {
                hello = create_body();
                if (hello) {
                    inet_ntop(AF_INET6, n->addr->sin6_addr.s6_addr, ipstr,
                              INET6_ADDRSTRLEN);

                    sprintf(msg,
                            "You recommended (%s, %u) but I can't reach it.",
                            ipstr, ntohs(*(u_int16_t *)(n->tutor_id + 16)));
                    cprint(0, "%s\n", msg);

                    hello->size =
                        tlv_warning(&hello->content, msg, strlen(msg));
                    push_tlv(hello, m);
                } else
                    cperror("malloc");
            }
        ...
        if (!hashset_remove_neighbour(potential_neighbours, n))
            cprint(STDERR_FILENO,
                   "%s:%d Tried to remove a potential neighbour but it wasn't "
                   "in the potential neighbour set.\n");
        free(n->addr);
        free(n->tutor_id);
        free(n);
    }
...
int hello_neighbours(struct timespec *tv) {
            ...
            if (now - p->last_hello < SYM_TIMEOUT) {
                ...
                if (delta >= MAX_TIMEOUT) {
                    ...
                    if (rc < 0) {
                        cprint(
                            STDERR_FILENO,
                            "Could not insert long hello into message queue\n");
                        free(hello->content);
                        free(hello);
                    }
    ...
    while (to_delete) {
        ...
        if (!hashset_remove(neighbours, p->addr->sin6_addr.s6_addr,
            ...
            cprint(
                STDERR_FILENO,
                "%s:%d Tried to remove a neighbour that wasn't actually one.\n",
                __FILE__, __LINE__);
