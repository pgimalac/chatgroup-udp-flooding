...
short hashset_isempty(hashset_t *t) { return t->size == 0; }
...
static short resize(hashset_t *h, int capacity) {
    ...
    for (size_t i = 0; i < h->capacity; list_destroy(h->tab[i], 0), i++)
        for (list_t *l = h->tab[i]; l != NULL; l = l->next)
            list_add(&t[hash_neighbour_data(GET_IP(l->val), GET_PORT(l->val)) %
                        capacity],
                     l->val);
...
short hashset_contains(hashset_t *h, const u_int8_t ip[sizeof(struct in6_addr)],
                       u_int16_t port) {
...
neighbour_t *hashset_get(hashset_t *h,
    ...
    for (list_t *l = h->tab[hash_neighbour_data(ip, port) % h->capacity];
         l != NULL; l = l->next)
        if (port == GET_PORT(l->val) &&
            memcmp(GET_IP(l->val), ip, sizeof(struct in6_addr)) == 0) {
...
static void *hashset_list_remove(list_t **l,
                                 const u_int8_t ip[sizeof(struct in6_addr)],
                                 u_int16_t port) {
    if (l == NULL || *l == NULL)
        return NULL;

    if (GET_PORT((*l)->val) == port &&
        memcmp(GET_IP((*l)->val), ip, sizeof(struct in6_addr)) == 0)
        return list_remove(l, 0);

    list_t *tmp;
    for (tmp = *l; tmp->next != NULL && (GET_PORT(tmp->next->val) != port ||
                                         memcmp(GET_IP(tmp->next->val), ip,
...
neighbour_t *hashset_remove(hashset_t *h,
                            const u_int8_t ip[sizeof(struct in6_addr)],
                            u_int16_t port) {
    if (h == NULL)
        return NULL;
