...
static map_elem *get(hashmap_t *map, const void *key) {
    ...
    map_elem *e;
    for (list_t *l = map->tab[hash_key(key, map->keylen) % map->capacity]; l;
         l = l->next) {
        e = (map_elem *)l->val;
...
static short map_list_remove(list_t **lst, const void *key, size_t keylen,
                             short k, short v) {
    ...
    if (memcmp(key, ((map_elem *)(*lst)->val)->key, keylen) == 0) {
        ...
        *lst = (*lst)->next;
        ...
        while ((*lst)->next &&
               memcmp(key, ((map_elem *)(*lst)->next->val)->key, keylen))
            lst = &(*lst)->next;
