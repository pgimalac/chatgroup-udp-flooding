...
static void onsend_data(const u_int8_t *tlv, neighbour_t *dst,
    ...
    if (!datime)
        cprint(STDERR_FILENO,
               "%s:%d Tried to get a tlv from a data_map but it wasn't in.\n",
               __FILE__, __LINE__);
    else
        datime->last = now;
    ...
    time_t delay = (rand() % (1 << (dinfo->send_count + 2))) +
                   (1 << (dinfo->send_count + 1));
...
onsend_fnc onsenders[NUMBER_TLV_TYPE] = {
    onsend_pad1, onsend_padn, onsend_hello,  onsend_neighbour,
    onsend_data, onsend_ack,  onsend_goaway, onsend_warning};
...
int send_message(int sock, message_t *msg, struct timespec *tv) {
    ...
    for (p = msg->body; p; p = p->next) {
        if (p->content[0] >= NUMBER_TLV_TYPE)
            onsend_unknow(p->content, msg->dst, tv);
        else
            onsenders[(int)p->content[0]](p->content, msg->dst, tv);
    }
